				 ...
				 pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudA, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudB, int threshold)
{
	...
	tracker.match(imageA, imageB, matches, keypoints1, keypoints2);
	...
	for(int i = 0; i < matches.size() ; i++)
	{
		...
		cv::Point2f point2DA = keypoints1.at(matches.at(i).queryIdx).pt;
		cv::Point2f point2DB = keypoints2.at(matches.at(i).trainIdx).pt;
		if(point2DA.x < 0 || point2DA.y < 0 || point2DB.x < 0 || point2DB.y < 0)
			continue;
		pcl::PointXYZRGB pointA = cloudA->at(point2DA.x, point2DA.y);
		pcl::PointXYZRGB pointB = cloudB->at(point2DB.x, point2DB.y);
