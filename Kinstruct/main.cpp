				 ...
				 pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudA, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudB)
{
	...
	time_t before;
    before = time (NULL);
    int tracked = rmatcher.track(grayA, grayB);
	...
	after = time (NULL);
	cout << "Tracking took " << after - before << endl;
	if(tracked > 200)
		return true;
	...
	*result += *cloudA;
	...
	while(times < final)
	{
		...
		if(!tooClose)
		{
			...
			time_t afterICP = time (NULL);
			cout << "Found Transformation " << afterICP - beforeICP << endl;
			...
			after = time (NULL);
			total += after - before;
			cout << "Total of frames " << times - 1 << " with " << times << " is " << after - before << endl;
